...
import {
  ...
  Calendar, Hash, FileText, Download, Upload, RotateCcw
} from 'lucide-react';
import {
    ...
    <div className={`flex flex-col h-full ${isTab ? '' : 'p-6 bg-[#f8fafc] flex-1 min-h-0 overflow-hidden relative font-sans text-slate-900'}`}>
      {!isTab && (
        <div className="flex items-center mb-8">
          <div className="w-1/4">
            <h1 className="text-2xl font-black text-[#172B4D] tracking-tight uppercase">Accessories</h1>
          ...
          <div className="flex items-center justify-end gap-2 ml-auto">
            ...
            <button onClick={() => setModal({ mode: 'add' })} className="flex items-center gap-2 px-4 py-2 bg-[#0052CC] text-white rounded-xl font-bold text-xs shadow-md hover:bg-[#0747A6] transition-all active:scale-95 group">
              <Plus size={16} className="group-hover:rotate-90 transition-transform duration-300" /><span>Add Accessory</span>
            </button>
          </div>
        </div>
      )}
        ...
        <div>
          <div className="flex items-center gap-6 ml-auto justify-between h-15 border-b border-gray-50">
            <div className="flex items-center gap-3 px-5 py-2 flex-1">
              ...
              <select className="w-40 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-600 outline-none focus:border-[#0052CC]" value={typeFilter} onChange={e => setTypeFilter(e.target.value)}>
