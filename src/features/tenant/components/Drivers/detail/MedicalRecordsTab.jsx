...
import {
  ...
  Pencil, Trash2, Stethoscope, RefreshCw,
  X, ChevronDown,
} from 'lucide-react';
import {
...
const FITNESS_STYLES = {
  FIT:         { text: 'text-green-700', bg: 'bg-green-50 border border-green-200' },
  UNFIT:       { text: 'text-red-700',   bg: 'bg-red-50 border border-red-200' },
  CONDITIONAL: { text: 'text-orange-700', bg: 'bg-orange-50 border border-orange-200' },
};

const FITNESS_STATUS = ['FIT', 'UNFIT', 'CONDITIONAL'];
...
// ── Add Medical Modal ─────────────────────────────────────────────────
const AddMedicalModal = ({ driverId, onClose }) => {
  ...
  const createRecord = useCreateMedicalRecord(driverId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-black text-[#172B4D]">Add Medical Record</h2>
            <p className="text-xs text-gray-400 mt-0.5">Add a new medical record for this driver</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label required>Fitness Status</Label>
              <Sel value={form.fitness_status} onChange={set('fitness_status')}>
                {FITNESS_STATUS.map(s => <option key={s} value={s}>{s}</option>)}
              </Sel>
            </div>
            <div><Label>Blood Group</Label><Input placeholder="e.g. O+, A-, B+" value={form.blood_group} onChange={set('blood_group')} /></div>
            <div><Label>Examining Doctor</Label><Input placeholder="e.g. Dr. Smith" value={form.examining_doctor} onChange={set('examining_doctor')} /></div>
            <div><Label>Certificate Number</Label><Input placeholder="e.g. MED123456" value={form.certificate_number} onChange={set('certificate_number')} /></div>
          </div>
          <div><Label>Restrictions</Label><Input placeholder="e.g. No night driving" value={form.restrictions} onChange={set('restrictions')} /></div>
          <div><Label>Certificate URL</Label><Input placeholder="https://example.com/certs/med.pdf" value={form.certificate_url} onChange={set('certificate_url')} /></div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50">
          ...
          <button onClick={handleSubmit} disabled={!form.examination_date || !form.fitness_status || createRecord.isPending}
...
// ── Edit Medical Modal ────────────────────────────────────────────────
const EditMedicalModal = ({ record, driverId, onClose }) => {
  const [form, setForm] = useState({
    examination_date:   record.examination_date   ?? '',
    next_due_date:      record.next_due_date       ?? '',
    fitness_status:     record.fitness_status      ?? 'FIT',
  ...
  const [error, setError] = useState('');
  const updateRecord = useUpdateMedicalRecord(driverId, record.id);
  const set = (f) => (e) => setForm(p => ({ ...p, [f]: e.target.value }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-black text-[#172B4D]">Edit Medical Record</h2>
            <p className="text-xs text-gray-400 mt-0.5">Editing: <span className="font-semibold text-gray-600">{record.examination_date}</span></p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100"><X size={18} /></button>
        </div>
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
          <button onClick={handleSubmit} disabled={!form.examination_date || !form.fitness_status || updateRecord.isPending}
            className="flex items-center gap-2 px-5 py-2 text-sm font-bold text-white bg-[#0052CC] rounded-lg hover:bg-[#0043A8] disabled:opacity-50 disabled:cursor-not-allowed">
...
const DeleteConfirm = ({ record, onConfirm, onCancel, isDeleting }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
      ...
      <h3 className="text-base font-black text-[#172B4D] mb-1">Delete Medical Record?</h3>
      <p className="text-sm text-gray-400 mb-5">
        Record from <span className="font-semibold text-gray-600">{record.examination_date}</span> will be permanently deleted.
      </p>
...
const MedicalTab = ({ driverId }) => {
  ...
  if (isError) return (
    <div className="flex flex-col items-center justify-center py-16 gap-3 text-red-400">
      <AlertCircle size={32} />
      <p className="text-sm font-medium">Failed to load medical records</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['Exam Date','Next Due','Fitness','Blood Group','Doctor','Certificate No.','Restrictions','Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {records.map(rec => {
                const fs = FITNESS_STYLES[rec.fitness_status] ?? FITNESS_STYLES.FIT;
                return (
                  <tr key={rec.id} className="hover:bg-blue-50/30 transition-colors">
                    {/* Exam Date */}
                    <td className="px-4 py-3 whitespace-nowrap font-semibold text-[#172B4D] text-[13px]">{rec.examination_date ?? '—'}</td>

                    {/* Next Due Date */}
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`text-[12px] font-mono ${getDueDateColor(rec.next_due_date)}`}>{rec.next_due_date ?? '—'}</span>
                    </td>

                    {/* Fitness Status */}
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold w-fit ${fs.bg} ${fs.text}`}>
                        <span className="w-1.5 h-1.5 rounded-full bg-current opacity-70" />
                        {rec.fitness_status_display ?? rec.fitness_status}
                      </span>
                    </td>
                    ...
                    {/* Doctor */}
                    <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-600">{rec.examining_doctor ?? '—'}</td>

                    {/* Certificate Number */}
                    <td className="px-4 py-3 whitespace-nowrap">
                      {rec.certificate_number
                        ? <span className="font-mono text-[12px] text-gray-600">{rec.certificate_number}</span>
                        : <span className="text-[12px] text-gray-400">—</span>}
                    </td>

                    {/* Restrictions */}
                    <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-600 max-w-30 truncate">
                      {rec.restrictions ?? '—'}
                    </td>
