...
import EmergencyTab from './EmergencyContactsTab';
import TrainingTab from './TrainingRecordsTab';
import MedicalTab from './MedicalRecordsTab';
import PerformanceTab from './PerformanceMetricsTab';
import IncidentsTab from './IncidentsTab';
import AttendanceTab from './AttendanceTab';
import VehicleTab from './VehicleAssignmentsTab';
import SalaryTab from './SalaryStructureTab';
...
// ── Edit Driver Modal ─────────────────────────────────────────────────
const LICENSE_TYPES = ['HMV', 'LMV', 'TRANSPORT', 'MMV', 'LEARNER', 'INTERNATIONAL'];
const DRIVER_TYPES = ['PERMANENT', 'CONTRACT', 'TEMPORARY', 'PART_TIME'];
const DRIVER_STATUSES = ['ACTIVE', 'INACTIVE', 'ON_LEAVE', 'SUSPENDED', 'TERMINATED'];
...
const EditDriverModal = ({ driver, onClose }) => {
  ...
  const handleSubmit = () => {
    ...
    const { first_name, last_name, phone, email, date_of_birth, gender, ...driverFields } = form;
    ...
    updateDriver.mutate({
      ...
      data: {
        ...clean(driverFields),
        user: clean({ first_name, last_name, phone, email, date_of_birth, gender }),
      },
    }, {
      onSuccess: onClose,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-black text-[#172B4D]">Edit Driver</h2>
            <p className="text-xs text-gray-400 mt-0.5">Update driver information</p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>License Type</Label>
                <Sel value={form.license_type} onChange={set('license_type')}>
                  <option value="">— Select —</option>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Driver Type</Label>
                <Sel value={form.driver_type} onChange={set('driver_type')}>
                  <option value="">— Select —</option>
